...
def fit_country_seiirr(country, population,bounds=None):
    N = population

    xdata,ydata,Ti=get_data(country)
    res_const = minimize(eval_model_seiirr,([1., 1. , 10., 10., 10.]), bounds=bounds,args=(xdata,ydata,Ti, population),method='L-BFGS-B')

    msle_cases,sol=eval_model_seiirr(res_const.x,xdata,ydata,Ti,population,return_sol=True)
    
    p,beta,Td,Tr,Tl=res_const.x
    ...
    R0 = beta*(p* Td + (1-p)*Tr)
    R0obs = beta*p*Td
    la = country + ": p = %.2f, "%p +r" $\beta$ =  %3.2f, " %beta + 'MSLE = %.3f \n '%msle_cases + '$R_0$ = %.1f, '%R0 + '$R_0^o$ = %.1f, '%R0obs + '$T_i$ = %d \n'%Ti+ '$T_d,T_r,T_l$ =%d, %d, %d'%(Td,Tr,Tl) 
...
bounds1=((0.15,.75),(0.5,2.5), (2,15),(4,15),(3,14)) # ((pmin,pmax),(beta_min, beta_max),Td(min,max),Tr(min,max),Tl(min,max) )
...
plt.grid()
plt.tight_layout()
plt.savefig('p_beta_Spain_Germany_v2_fit_all.pdf')
